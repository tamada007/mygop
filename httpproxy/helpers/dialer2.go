...
func (d *MultiDialer) dialMultiTLS(network string, hosts []string, port string, config *tls.Config) (net.Conn, error) {
	...
	for _, host := range hosts {
		go func(host string, c chan<- connWithError) {
			...
			_, err := net.ResolveTCPAddr(network, net.JoinHostPort(host, port))
			if err != nil {
			...
			//			conn, err := net.DialTCPContext(ctx, network, nil, raddr) //kage
			conn, err := dialer.DialContext(ctx, network, network) //kage
...
func (d *MultiDialer) DialQuic(address string, tlsConfig *tls.Config, cfg *quic.Config) (quic.Session, error) {
	...
	if cfg == nil {
		cfg = &quic.Config{
			...
			//			RequestConnectionIDTruncation: true,	//kage
			KeepAlive: true,
		}
	}
